...
namespace ngine::graphics {
     ...
     * Text wrapping mode.
     */
    enum class TextWrap { None, BreakLine, BreakWord };

    /**
     * Multiplier for the space between lines
     */
    enum class LineSpacing { One, OnePointFive, Two };
    ...
    struct NEAPI CharInfo {
         ...
         */
        CharInfo() : Character(0), OffsetX(0), OffsetY(0), AdvanceX(0) {}
    };
    ...
    public:
         ...
         *
         * @warning The higher the base size is set, the more memory each loaded
         * font will use.
         * @param path Path to font file.
         * @param baseSize Base font size.
         * @param fontChars The list of characters to add into the atlas. Leave
         * empty for standard alpha-numeric characters
         * @return The loaded font.
         */
        static Font *
        loadTTFFont(GraphicsDevice *graphicsDevice,
                    const filesystem::Path &path, int baseSize = 36,
         ...
         */
        void drawString(Renderer *renderer, const std::string &string,
                        const Vector2 &position, float fontSize, float spacing,
                        const Color &color,
        ...
        /**
         * Draw a string within given bounds with options for alignment and
         * wrapping.
         *
         * @warning Not implemented.
         */
        void drawString(Renderer *renderer, const std::string &string,
                        const Rectangle &bounds, float fontSize, float spacing,
                        const Color &color,
                        LineSpacing lineSpacing = LineSpacing::OnePointFive,
         ...
         */
        Vector2
        measureString(const std::string &string, float fontSize, float spacing,
                      LineSpacing lineSpacing = LineSpacing::OnePointFive);
         ...
         */
        Vector2
        measureStringRestrictedW(const std::string &string, float fontSize,
                                 float spacing, float maxWidth,
                                 TextAlignment alignment = TextAlignment::Left,
                                 TextWrap wrap = TextWrap::None);

        /**
         * Measure the size of a string if it was rendered with a maximum
         * height.
         *
         * @warning Not implemented.
         */
        Vector2
        measureStringRestrictedH(const std::string &string, float fontSize,
                                 float spacing, float maxHeight,
                                 TextAlignment alignment = TextAlignment::Left,
                                 TextWrap wrap = TextWrap::None);
    ...
    private:
         ...
         */
        void _loadFontInfo(const filesystem::Path &path,
