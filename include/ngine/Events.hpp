...
namespace ngine {
     ...
     * Events do not support polymorphism, only hooking the exact type will
     * work.
     */
    class Event {
    public:
    ...
    public:
         ...
         *
         * @warning If this is not called and the `EventRef` is destroyed, you
         * cannot remove the function from the dispatcher.
         */
        void remove();
    ...
    public:
         ...
         * @param senderFilter Filter for senders, this will allow you to only
         * recieve events for a given sender.
         * @return A reference to the event binding.
         */
        template <typename TargetEvent>
        static EventRef
        bind(const std::function<void(const TargetEvent &e)> &fn,
             void *senderFilter = nullptr) {
            auto it =
                m_routes.emplace(&typeid(TargetEvent),
         ...
         */
        template <typename EventType> static void fire(const EventType &e) {
            auto range = m_routes.equal_range(&typeid(EventType));
            for (auto it = range.first; it != range.second; it++) {
                if (it->second->Filter == nullptr ||
                    it->second->Filter == e.Sender)
    ...
    private:
         ...
         */
        template <typename TargetEvent> class EventHook : public BaseEventHook {
        ...
        public:
             ...
             * @param senderFilter The sender filter
             */
            EventHook(std::function<void(const TargetEvent &)> fn,
                      void *senderFilter)
        ...
        /**
         * Unbind an event if you still have the function, otherwise use the
         * `EventRef`.
         *
