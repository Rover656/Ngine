...
namespace ngine::input {
    ...
    void Keyboard::_GLFWKeyCallback(GLFWwindow *window, int key, int scancode,
        ...
        if (action == GLFW_PRESS) {
            EventDispatcher::fire(
                KeyPressEvent(kbd, kbd->m_window, (Key)key,
            ...
            EventDispatcher::fire(KeyReleaseEvent(
                kbd, kbd->m_window, (Key)key,
                (mods & GLFW_MOD_SHIFT) == GLFW_MOD_SHIFT,
                (mods & GLFW_MOD_CONTROL) == GLFW_MOD_CONTROL,
                (mods & GLFW_MOD_ALT) == GLFW_MOD_ALT,
                (mods & GLFW_MOD_SUPER) == GLFW_MOD_SUPER,
    ...
    Key Keyboard::_keyFromVirtualKey(int key) {
        ...
        case VirtualKey::Back:
            actualKey = Key::Backspace;
            break;
        case VirtualKey::Insert:
            actualKey = Key::Insert;
            break;
        case VirtualKey::Delete:
            actualKey = Key::Delete;
            break;
        ...
        case VirtualKey::PageUp:
            actualKey = Key::PageUp;
            break;
        case VirtualKey::PageDown:
            actualKey = Key::PageDown;
            break;
        ...
        case VirtualKey::CapitalLock:
            actualKey = Key::CapsLock;
            break;
        case VirtualKey::Scroll:
            actualKey = Key::ScrollLock;
            break;
        case VirtualKey::NumberKeyLock:
            actualKey = Key::NumLock;
            break;
        // No print screen
        ...
        case VirtualKey::LeftWindows:
            actualKey = Key::LeftSuper;
            break;
        case VirtualKey::RightShift:
            actualKey = Key::RightShift;
            break;
        ...
        case VirtualKey::NumberPad0:
            actualKey = Key::NumPad0;
            break;
        case VirtualKey::NumberPad1:
            actualKey = Key::NumPad1;
            break;
        case VirtualKey::NumberPad2:
            actualKey = Key::NumPad2;
            break;
        case VirtualKey::NumberPad3:
            actualKey = Key::NumPad3;
            break;
        case VirtualKey::NumberPad4:
            actualKey = Key::NumPad4;
            break;
        case VirtualKey::NumberPad5:
            actualKey = Key::NumPad5;
            break;
        case VirtualKey::NumberPad6:
            actualKey = Key::NumPad6;
            break;
        case VirtualKey::NumberPad7:
            actualKey = Key::NumPad7;
            break;
        case VirtualKey::NumberPad8:
            actualKey = Key::NumPad8;
            break;
        case VirtualKey::NumberPad9:
            actualKey = Key::NumPad9;
            break;
        case VirtualKey::Add:
            actualKey = Key::NumPadAdd;
            break;
        case VirtualKey::Subtract:
            actualKey = Key::NumPadSubtract;
            break;
        case VirtualKey::Multiply:
            actualKey = Key::NumPadMultiply;
            break;
        case VirtualKey::Divide:
            actualKey = Key::NumPadDivide;
            break;
    ...
    void Keyboard::_UWPKeyDown(Windows::UI::Core::CoreWindow ^ sender,
                               Windows::UI::Core::KeyEventArgs ^ args) {
        Key key = _keyFromVirtualKey((int)args->VirtualKey);
        m_UWPKeyboard->m_nextState.setKeyState(key, true);

        // Fire event
        EventDispatcher::fire(KeyPressEvent(
            m_UWPKeyboard, m_UWPKeyboard->m_window, (Key)key,
            (sender->GetKeyState(Windows::System::VirtualKey::Shift) &
             CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down,
            (sender->GetKeyState(Windows::System::VirtualKey::Control) &
             CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down,
            false, // No alt
            (sender->GetKeyState(Windows::System::VirtualKey::LeftWindows) &
             CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down ||
                (sender->GetKeyState(
                     Windows::System::VirtualKey::RightWindows) &
                 CoreVirtualKeyStates::Down) ==
                    CoreVirtualKeyStates::Down, // Annoyingly, we need to check
                                                // both
            (sender->GetKeyState(Windows::System::VirtualKey::CapitalLock) &
             CoreVirtualKeyStates::Locked) == CoreVirtualKeyStates::Locked,
            (sender->GetKeyState(Windows::System::VirtualKey::NumberKeyLock) &
             CoreVirtualKeyStates::Locked) == CoreVirtualKeyStates::Locked));
    }

    void Keyboard::_UWPKeyUp(Windows::UI::Core::CoreWindow ^ sender,
                             Windows::UI::Core::KeyEventArgs ^ args) {
        Key key = _keyFromVirtualKey((int)args->VirtualKey);
    ...
    Keyboard::Keyboard(Window *window) : m_window(window) {
        ...
        if (m_UWPKeyboard != nullptr)
            throw std::runtime_error(
                "On UWP only one keyboard can be created.");
        ...
        UWPwindow->KeyDown += ref new Windows::Foundation::TypedEventHandler<
            Windows::UI::Core::CoreWindow ^, Windows::UI::Core::KeyEventArgs ^>(
            &_UWPKeyDown);
        UWPwindow->KeyUp += ref new Windows::Foundation::TypedEventHandler<
            Windows::UI::Core::CoreWindow ^, Windows::UI::Core::KeyEventArgs ^>(
            &_UWPKeyUp);
#endif
    }

    const KeyboardState &Keyboard::getState() const { return m_currentState; }
    ...
    bool Keyboard::isKeyDown(char key) const { return isKeyDown(getKey(key)); }

    bool Keyboard::isKeyPressed(Key key) const {
        return m_currentState.getKeyState(key) &&
               !m_previousState.getKeyState(key);
    }
