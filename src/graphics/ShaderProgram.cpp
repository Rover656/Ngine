...
namespace ngine::graphics {
    ShaderProgram::ShaderProgram(GraphicsDevice *graphicsDevice,
        ...
        if (VertexShader->Type != ShaderType::Vertex)
            Console::fail("ShaderProgram",
                          "Vertex shader is not a vertex shader.");

        if (FragmentShader->Type != ShaderType::Fragment)
            Console::fail("ShaderProgram",
                          "Fragment shader is not a fragment shader.");
    ...
    void ShaderProgram::addUniform(ShaderDataStructure uniform) {
        ...
        if (uniform.Offset != -1)
            Console::warn("ShaderProgram",
                          "Uniform offset has been set outside of the program, "
    ...
    void ShaderProgram::finalize() { m_final = true; }

    bool ShaderProgram::isFinal() const { return m_final; }

    bool ShaderProgram::isValid() const {
        return m_API->isShaderProgramValid(this);
    }

    void ShaderProgram::free() { m_API->deleteShaderProgram(this); }
} // namespace ngine::graphics
