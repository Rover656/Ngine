...
namespace ngine::graphics {
    RenderTarget::RenderTarget(GraphicsDevice *graphicsDevice,
        ...
        m_texture = new Texture2D(graphicsDevice, nullptr, width, height,
                                  UNCOMPRESSED_R8G8B8A8, 1);

        // Verify texture
        if (!m_texture->isValid()) {
            Console::error(
                "RenderTarget",
                "Failed to create color attachment for render target.");
    ...
    const Viewport *RenderTarget::getViewport() const { return &m_viewport; }
    ...
    Texture2D *RenderTarget::getTexture() { return m_texture; }
