...
namespace ngine::graphics {
    ShaderProgramState::ShaderProgramState(ShaderProgram *program)
        ...
        if (!AttachedProgram->isFinal())
            Console::fail("ShaderProgramState",
                          "Shader program must be marked as final before use.");
    ...
    const void *ShaderProgramState::getUniform(const std::string &name) {
        ...
        if (uniformDesc.getSize() == 0 || uniformDesc.Count == 0)
            Console::fail("ShaderUniformData",
                          "Uniform could not be found or uniform has invalid "
                          "parameters.");
    ...
    void ShaderProgramState::setUniform(const std::string &name, void *data) {
        if (m_data == nullptr)
            Console::fail("ShaderUniformData",
                          "Memory not allocated. Call Allocate() first.");
        ...
        // Copy data in
        char *dat = (char *)m_data;
        memcpy(&dat[uniformDesc.Offset], data,
               uniformDesc.getSize() * uniformDesc.Count);
    }

    void ShaderProgramState::setUniformEx(std::vector<std::string> nameTree,
                                          void *data) {
        ...
        if (target.getSize() == 0 || target.Count == 0)
            return;
        ...
        for (auto i = 1; i < nameTree.size(); i++) {
            ...
            case ShaderDataType::Matrix:
                Console::fail("ShaderProgramState",
                              "Cannot navigate *into* a simple data format.");
                break;
    ...
    const void *ShaderProgramState::getData() const { return m_data; }
} // namespace ngine::graphics