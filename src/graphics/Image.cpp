...
namespace ngine::graphics {
    void Image::_create(unsigned char *pixelData, unsigned int width,
        ...
        if (format == UNCOMPRESSED_GRAYSCALE)
            ...
            bpp = 2;
        else if (format == UNCOMPRESSED_R8G8B8)
            bpp = 3;
        else if (format == UNCOMPRESSED_R8G8B8A8)
            bpp = 4;
        else
        ...
        memcpy(PixelData, pixelData,
               width * height * sizeof(unsigned char) * bpp);
    ...
    Image::Image(const filesystem::Path &path) {
        ...
        if (ext == "png" || ext == "bmp" || ext == "tga" || ext == "jpg" ||
            ...
            auto data = stbi_load_from_file(file.getFileHandle(), &width,
                                            &height, &bpp, 4);
    ...
    Image::Image(unsigned char *pixelData, unsigned int width,
                 unsigned int height, PixelFormat format) {
    ...
    void Image::free() {
        // Removes image data from the container (across all copies of this
        // image)
