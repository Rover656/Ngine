...
namespace ngine::graphics {
    ...
    Font *Font::getDefaultFont() { return m_defaultFont; }
    ...
    Texture2D *Font::getTexture() const { return m_texture; }

    bool Font::isValid() const { return m_texture->isValid(); }

    Font *Font::loadTTFFont(GraphicsDevice *graphicsDevice,
                            const filesystem::Path &path, int baseSize,
                            std::vector<int> fontChars) {
        // Check size
        if (baseSize < 1)
            throw std::runtime_error("Invalid base size.");
    ...
    void Font::setDefaultFont(Font *font) { m_defaultFont = font; }
    ...
    void Font::drawString(Renderer *renderer, const std::string &string,
                          const Vector2 &position, float fontSize,
                          float spacing, const Color &color,
                          LineSpacing lineSpacing, const Angle &rotation,
        ...
        if (!willFit) // TODO: Automatically batch text for the developer
            Console::fail("Font", "Too much text for a single buffer, please "
                                  "split your draw calls up.");
        ...
        if (renderer->getCoordinateSystem() == CoordinateSystem::GUI)
            ...
            renderer->translate({position.X, position.Y - lineHeight, 0});
            ...
            if (letter == '\n') {
                ...
                if (letter != ' ') {
                    ...
                    if (renderer->getCoordinateSystem() ==
                        CoordinateSystem::GUI)
                        renderer->translate(
                            {textX + m_characters[index].OffsetX * scaleFactor,
                             textY + m_characters[index].OffsetY * scaleFactor,
                             0});
                    else
                        renderer->translate(
                            {textX + m_characters[index].OffsetX * scaleFactor,
                    ...
                    auto width =
                        m_characters[index].Rectangle.Width * scaleFactor;
                    auto height =
                        m_characters[index].Rectangle.Height * scaleFactor;
                    ...
                    renderer->pushVertex({{x1, y1, 0}, {srcX1, srcY1}, color});
                    renderer->pushVertex({{x1, y2, 0}, {srcX1, srcY2}, color});
                    renderer->pushVertex({{x2, y2, 0}, {srcX2, srcY2}, color});
                    renderer->pushVertex({{x2, y1, 0}, {srcX2, srcY1}, color});
                ...
                if (m_characters[index].AdvanceX == 0)
                    textX += ((float)m_characters[index].Rectangle.Width *
                                  scaleFactor +
                              spacing);
                else
                    textX +=
                        ((float)m_characters[index].AdvanceX * scaleFactor +
                         spacing);
            }
        }
    ...
    void Font::drawString(Renderer *renderer, const std::string &string,
                          const Rectangle &bounds, float fontSize,
                          float spacing, const Color &color,
                          LineSpacing lineSpacing, TextAlignment alignment,
                          TextWrap wrap, const Angle &rotation,
                          const Vector2 &origin) {
        Console::fail("Font",
                      "This version of DrawString is not yet implemented.");
    }

    Vector2 Font::measureString(const std::string &string, float fontSize,
                                float spacing, LineSpacing lineSpacing) {
        // Get ready to process text
        float textX = 0.0f, textY = 0.0f,
              scaleFactor = fontSize / (float)m_baseSize, maxX = 0.0f;
    ...
    Vector2 Font::measureStringRestrictedW(const std::string &string,
                                           float fontSize, float spacing,
                                           float maxWidth,
                                           TextAlignment alignment,
                                           TextWrap wrap) {
        Console::fail("Font",
                      "MeasureStringRestrictedW is not yet implemented.");
        return {};
    }

    Vector2 Font::measureStringRestrictedH(const std::string &string,
                                           float fontSize, float spacing,
                                           float maxHeight,
                                           TextAlignment alignment,
                                           TextWrap wrap) {
        Console::fail("Font",
                      "MeasureStringRestrictedH is not yet implemented.");
        return {};
    }
    ...
    void Font::_generateAtlas(GraphicsDevice *graphicsDevice) {
        ...
        for (int i = 0; i < m_characterCount; i++) {
            ...
            for (int y = 0; y < m_characters[i].Image->Height; y++) {
                for (int x = 0; x < m_characters[i].Image->Width; x++) {
                    ((unsigned char *)
                         atlas->PixelData)[(offsetY + y) * atlas->Width +
                                           (offsetX + x)] =
                        ((unsigned char *)m_characters[i]
                             .Image
                             ->PixelData)[y * m_characters[i].Image->Width + x];
                }
            }
            ...
            if (offsetX >=
                (atlas->Width - m_characters[i].Image->Width - padding)) {
                offsetX = padding;

                // NOTE: Be careful on offsetY for SDF fonts, by default SDF
                // use an internal padding of 4 pixels, it means char rectangle
                // height is bigger than fontSize, it could be up to (fontSize +
                // 8)
                offsetY += (maxH + 2 * padding);

                maxH = 0;

                if (offsetY > (atlas->Height - m_baseSize - padding))
                    break;
            }
        }
        ...
        unsigned char *dataGrayAlpha =
            new unsigned char[atlas->Width * atlas->Height * 2];
    ...
    void Font::_loadFontInfo(const filesystem::Path &path,
        ...
        if (fontFile.open(filesystem::FileOpenMode::Read, true)) {
            ...
            auto scaleFactor =
                stbtt_ScaleForPixelHeight(&fontInfo, (float)m_baseSize);
            ...
            for (auto i = 0; i < m_characterCount; i++) {
                ...
                // Get image data
                auto pixels = stbtt_GetCodepointBitmap(
                    &fontInfo, scaleFactor, scaleFactor, ch, &chw, &chh,
                    &m_characters[i].OffsetX, &m_characters[i].OffsetY);

                // Load image
                m_characters[i].Image = std::make_shared<Image>(
                    pixels, chw, chh, UNCOMPRESSED_GRAYSCALE);

                // Set offset
                m_characters[i].OffsetY += (int)((float)ascent * scaleFactor);

                // Get advance
                stbtt_GetCodepointHMetrics(&fontInfo, ch,
                                           &m_characters[i].AdvanceX, nullptr);
